...
const { 
    Permissions, Client, Attachment, MessageCollector, Interaction, MessageEmbed, 
    MessageActionRow, MessageSelectMenu, MessageButton, Intents, Modal, 
    TextInputComponent, showModal 
} = require('discord.js');
...
if (currentBotMode == 'test') {
    ...
    levelDirectory = "./level/"
    savesDirectory = "./saves/"
    VideoCallSendChannelID = CytusHeardleWareHouseID
        ...
        switch(commandName) {
            ...
            case 'score':
                ...
                if (sentChannelId == CytusHeardleScoresChannelID) {
                    let targetChannel = await client.channels.fetch(CytusHeardleWareHouseID)
...
client.on('messageCreate', async (message)=>{
    ...
    switch(args[0]){
        ...
        case 'optin':
            ...
            break;   
            case 'play':
                const directory = message.content.substring(6).trim();
                ...
                if (!songQueue.has(guildId)) {
                    ...
                    connection.on('stateChange', (oldState, newState) => {
                        if (newState.status === 'disconnected') {
                            console.log(`Voice connection for ${guildId} disconnected.`);
                    
                            const serverQueue = songQueue.get(guildId);
                            if (serverQueue) {
                                serverQueue.player.stop(); // Just in case
                                songQueue.delete(guildId);
                            }
                        }
                    });
                    ...
                    songQueue.get(guildId).songs.push(filePath);
                    message.channel.send("Song added to queue!");
                }
            
                break;
            case 'skip':
                serverQueue = songQueue.get(message.guild.id);
                ...
                message.channel.send(`${ScoreSkip} Skipping the current song!`);
                break;
            case 'queue':
                serverQueue = songQueue.get(message.guild.id);
                ...
                // Format the queue into a readable list
                const queueMessage = serverQueue.songs
                    .map((song, index) => {
                        const fileName = path.basename(song);
                        return `${index === 0 ? `${projectDivaMiku}Now Playing:` : `#${index}`} ${fileName}`;
                    })
                    .join('\n');

                message.channel.send(`${angryMiku} **Current Queue:**\n` + queueMessage);
                break;
            case 'uploadfile':
                ...
                if (!originalName.endsWith('.mp3')) {
                    return message.reply(`${tick} Only `.mp3` files are supported.`);
                }

                // Ask the user for a name
                await message.reply(`${gemz} What should I name this file? Please reply within 30 seconds.`);

                filter = response => response.author.id === message.author.id;
                try {
                    const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
                    const nameResponse = collected.first().content;
                    const sanitizedName = nameResponse.replace(/[^a-zA-Z0-9_\- ]/g, "_"); // clean filename
                    const fileName = sanitizedName.endsWith('.mp3') ? sanitizedName : sanitizedName + '.mp3';
                    ...
                    message.channel.send(`${Hehehebutinreallife} File saved as \`./songs/${fileName}\``);
                    const userId = message.author.id;
                    if (!userUploads[userId]) userUploads[userId] = [];
    
                    userUploads[userId].push(fileName);
    
                    // Save to disk
                    fs.writeFileSync(uploadDataPath, JSON.stringify(userUploads, null, 2));
                } catch (err) {
                    console.error(err);
                    message.reply("⌛ You didn't reply in time. Upload cancelled.");
                }
            ...
            case 'songs':
                ...
                for (const uploads of Object.values(userUploads)) {
                    uploads.forEach(song => allSongsSet.add(song));
                }

                const allSongs = [...allSongsSet];

                if (allSongs.length === 0) {
                    return message.reply("📭 No songs have been uploaded yet.");
                }

                const allSongList = allSongs.map(s => `- ${s}`).join('\n');
                message.reply(`🎶 All songs:\n\`\`\`\n${allSongList}\n\`\`\`\nUse \`-play ./songs/<filename>\` to play one.`);
                break;
            case 'upload':
                ...
                if (args.length < 2) {
                    return message.reply("❌ Please provide a YouTube link like `!upload <url>`");
                }

                const ytUrl = args[1];

                // Validate URL
                if (!play.yt_validate(ytUrl)) {
                    return message.reply("❌ That doesn't look like a valid YouTube URL.");
                }

                // Get video info
                let info;
                try {
                    info = await play.video_info(ytUrl);
                } catch (err) {
                    console.error(err);
                    return message.reply("⚠️ Couldn't fetch video info. YouTube may have changed something.");
                }

                const videoTitle = info.video_details.title;
                await message.reply(`🎬 Found: **${videoTitle}**\nWhat should I name this file? (reply within 30 seconds)`);
                ...
                try {
                    const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
                    const nameResponse = collected.first().content;
                    const sanitizedName = nameResponse.replace(/[^a-zA-Z0-9_\- ]/g, "_");
                    const fileName = sanitizedName.endsWith('.mp3') ? sanitizedName : sanitizedName + '.mp3';
                    const savePath = path.join(__dirname, 'songs', fileName);
                    ...
                    message.channel.send("🎶 Downloading and converting to MP3...");
                    ...
                    .on('end', () => {
                        message.channel.send(`✅ Saved as \`./songs/${fileName}\``);
                        const userId = message.author.id;
                        if (!userUploads[userId]) userUploads[userId] = [];
                        userUploads[userId].push(fileName);
                        fs.writeFileSync(uploadDataPath, JSON.stringify(userUploads, null, 2));
                    ...
                    .on('error', err => {
                        console.error(err);
                        message.reply("⚠️ Something went wrong during download or conversion.");
                    })
                    .save(savePath);
                ...
                // await message.reply(`🎬 Found: **${videoTitle}**\n${gemz} What should I name this file? (reply within 30 seconds)`);
                ...
                //     const stream = ytdl(ytUrl, { quality: 'highestaudio' });
